...
function parseBadges(markdown, title) {
  ...
  for (const badge of badges) {
    if (!badge.image.startsWith("https://flat.badgen.net/")) {
      throw new Error(`Badge image in "${title}" does not use flat.badgen.net: ${badge.image}`);
    }
    if (!badge.alt.endsWith(".")) {
      throw new Error(`Badge alt text in "${title}" must end with a period: ${badge.alt}`);
    }
  }
...
function parseLibrary(markdown) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (index >= lines.length) {
      throw new Error(`Unclosed Markdown fence starting at library.md:${blockStartLine - 1}.`);
    }

    const template = block.join("\n").trim();
    const entryTitle = title || category;
    if (!category || !entryTitle) {
      throw new Error(`Layout at library.md:${blockStartLine} has no section heading.`);
    }
    ...
    const placeholders = placeholderNames
      ...
      .sort((left, right) => template.indexOf(left) - template.indexOf(right));
    ...
    if (title) {
      headingLayoutCounts.set(headingLine, (headingLayoutCounts.get(headingLine) ?? 0) + 1);
    }
    intro = [];
  }

  if (entries.length !== markdownFenceCount) {
    throw new Error(`Parsed ${entries.length} layouts from ${markdownFenceCount} Markdown fences.`);
  }
  if (entries.length < minimumLayoutCount) {
    throw new Error(`Expected at least ${minimumLayoutCount} layouts, found ${entries.length}.`);
  }

  const emptyHeadings = [...headingLayoutCounts.entries()].filter(([, count]) => count === 0);
...
const badgeCount = entries.reduce((total, entry) => total + entry.badgeCount, 0);
const catalog = {
...
if (checkOnly) {
  ...
  if (current !== output) {
    throw new Error("docs/catalog.js is stale. Run npm run build and commit the result.");
  }
  console.log(
